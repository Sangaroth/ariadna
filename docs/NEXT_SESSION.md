...
- **✅ HECHO Y VERIFICADO (2026-05-30 noche) — IdeaBlock first-class (persistir sumario + indexar chunks Layer 0).**
  - **Persistir** (`ariadna/project_config.py`): `self.summaries_dir = self.root / "summaries"`. **`ariadna/ideablocks.py`** (nuevo): `sanitize/summary_path/write_summary/read_summary/iter_summaries`. Sumario en `projects/<slug>/summaries/<sanitize(source_id)>.md` con frontmatter (`source_id, source_type, title, generated_at`) + cuerpo (índice de IdeaBlocks tal cual lo emite el summarizer).
  - **Reusar** (`scripts/worker.py:process_item`): si existe el summary persistido → reusarlo (`ingest_method="reused_summary"`) y **saltar acquire+summarize** (cero LLM, cero descarga); si no → bypass/generar → **persistir** antes de `register_source`. El return expone `reused_summary` + `summary_path`.
  - **Indexar Layer 0** (`ariadna/ideablocks.py:index_project_chunks(project)`): lee `summaries/*.md` → `adapter.parse_summary_to_chunks` → embed (DenseEmbedder) → Qdrant upsert con payload universal + `embedding_role="chunk"`; idempotencia `delete_by_filter({project_id, embedding_role:"chunk"})`. Llamada desde `worker.index_batch` tras el index wiki (secuencial, server parado).
  - **BUG resuelto — colisión de IDs**: un paper tiene N IdeaBlocks en la MISMA página → `chunk_id` (= `source_id#pN`) colisiona (17 chunks → solo 8 IDs distintos). Fix: el ID entero del punto Qdrant desambigua con ordinal por fuente `sha256("chunk:"+chunk_id+"#b<n>")` + payload `block_index`; `position`/`cite_markdown` siguen a granularidad de página (`pN`) para que el lookup de citas case ambos bloques. Invariante anti-colisión en el upsert.
  - **search.py**: `SearchResult.from_payload` ahora **TOLERANTE** — chunks de paper sin `video_id/video_title/timestamp/youtube_url/category/playlist` caen a `source_id/title/position_url` vía `.get()`. (Mejor a futuro: universalizar `SearchResult`.)
  - **Verificación E2E (server real, Qdrant real)**: regenerado el sumario del paper metacognición vía worker (hash-bypass, `claude -p` summarize) → **persistido** (`doi_10.3389_fnhum.2014.00443.md`, 17 IdeaBlocks) → `worker.py --index atlas-teleosemantico` → **17 chunks Layer 0 indexados** (cero colisión). `search_corpus(project=["atlas-teleosemantico"], query="meta-d prime…")` devuelve **5 raw_chunks del paper** (score 0.96→0.69, cite_markdown `[How to measure metacognition, p.N](doi#page=N)`). Isolation OK (`project="proxy"` → 0 fugas de atlas). test_hybrid 8/8, test_worker 5/5, test_ideablocks 2/2.
  - **Nota**: la re-extracción del worker añadió 5 páginas wiki NUEVAS a atlas (stephen-fleming, blindsight-tipo-2, calibracion-de-la-confianza, d-prime-tipo-2, how-to-measure-metacognition) sin tocar las 17 previas. Q&A contextualizada sigue **DESCARTADA** (revisitar solo tras eval).
