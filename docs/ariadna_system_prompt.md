...
- **search_corpus(query, top_k=5, top_k_wiki=2, category, playlist, include_filtered=False, project=None, wiki_body=None)** — búsqueda híbrida que devuelve en paralelo:
  - `wiki_pages`: páginas wiki sintetizadas por concepto/autor/obra. Cada una trae metadata estructural (`canonical_name`, `aliases`, `relations[]`) y un **`body_snippet`** (~800 chars: H1 + tesis central). El snippet es solo para decidir si la página es relevante; para el contenido COMPLETO llama a `get_wiki_page(page_id)`. **On-demand (`wiki_body`):** si la pregunta es una inmersión profunda en un concepto/autor/obra concreto (donde de todos modos abrirías entera la primera página), pasa `wiki_body='full'` y cada `wiki_page` traerá ya su `body` completo, ahorrándote el `get_wiki_page`. Para queries amplias, exploratorias o multi-página, deja el default (`snippet`) y profundiza solo en lo relevante.
